...
public:
  MbfBridge() {
    ...
    ros::NodeHandle nh("/");
    keep_out_zone_srv_ = nh.advertiseService("ackermann_zone", &MbfBridge::keepOutZoneSrv, this);
    local_keep_out_ = nh.serviceClient<costmap_2d::keepOutZone>(
      "/move_base_flex/local_costmap/keep_out_layer/ackermann_zone");
    global_keep_out_ = nh.serviceClient<costmap_2d::keepOutZone>(
      "/move_base_flex/global_costmap/keep_out_layer/ackermann_zone");
  }
...
int main(int argc, char** argv) {
  ...
  ackermann_planning::pnc::MbfBridge _bridge;
  ros::spin();
