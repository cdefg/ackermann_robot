...
namespace ackermann_planning::pnc {
...
void StateMachine::init() {
  ...
  coverage_srv_ = nh.serviceClient<coverage_path_planner::GetPathInZone>("/ackermann_zone_path");
...
auto StateMachine::task_service(ackermann_planning::task::Request& req, ackermann_planning::task::Response& resp) -> bool {
  std::string root_path;
  ROS_ERROR_COND(!ros::param::get("/root_path", root_path), "Get root path failed!");
  switch (req.type) {
    case ackermann_planning::task::Request::EXECUTE: {
      ...
      if (req.command == ackermann_planning::task::Request::PAUSE) {
      ...
      if (req.command == ackermann_planning::task::Request::STOP) {
      ...
      if (req.command == ackermann_planning::task::Request::START) {
    ...
    case ackermann_planning::task::Request::RECORD: {
      switch (req.command) {
        case ackermann_planning::task::Request::START: {
        ...
        case ackermann_planning::task::Request::KEEP_TEACH:
        case ackermann_planning::task::Request::KEEP_COVER_ZZ:
        case ackermann_planning::task::Request::KEEP_COVER_BS:
        case ackermann_planning::task::Request::DISCARD: {
          if (cur_state_ != StateValue::Record) {
          ...
          cur_state_ = StateValue::Idle;
          if (req.command == ackermann_planning::task::Request::DISCARD) {
            resp.message = "Do not keep path, discard recorded data.";
          } else {
            if (req.command == ackermann_planning::task::Request::KEEP_TEACH) {
              ...
              r.request.type = req.command == ackermann_planning::task::Request::KEEP_COVER_ZZ ? 0 : 1;
        ...
        case ackermann_planning::task::Request::KEEP_TRAFFIC_ROUTE : {
    ...
    case ackermann_planning::task::Request::LOAD_TRAFFIC_ROUTE: {
      lines_type lines;
      auto dir = root_path + "/map/";
      auto map_path = dir + req.map + "_traffic_route.txt";
    ...
    case ackermann_planning::task::Request::QR_NAV: {
      using goalState = actionlib::SimpleClientGoalState;
